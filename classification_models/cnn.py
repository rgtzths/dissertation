...
from wt import get_continuous_features
...
class CNN():
    def __init__(self, params):
        ...
        #If this variable isn't None than the class loads the appliance models present in the folder.
        self.load_model_path = params.get('load_model_folder',None)
        #Dictates the ammount of information to be presented during training and classification.
        self.verbose = params.get('verbose', 0)
        #Defines the window of time of each feature vector (in mins)
        self.timewindow = params.get('timewindow', {})
        #Defines the step between each reading (in seconds)
        ...
        #Name of the continuous wavelet to use.
    ...
    def partial_fit(self, train_mains, train_appliances):
        ...
        if not self.randomsearch:
            ...
            for app_name, appliance_power in train_appliances:
                ...
                X_train = get_continuous_features(train_data, self.waveletname)  
                ...
                if app_name in self.model:
                    ...
                    model = self.create_model((X_train.shape[1], X_train.shape[1], X_train.shape[3]))
    ...
    def disaggregate_chunk(self, test_mains, test_appliances):
        ...
        for app_name, appliance_power in test_appliances:
            ...
            X_test = get_continuous_features(test_data, self.waveletname) 
    ...
    def random_search(self, train_mains, train_appliances):
        ...
        for timewindow in self.randomsearch_params['timewindow']:
            for timestep in self.randomsearch_params['timestep']:
                ...
                self.randomsearch_params['model']['input_shape'] = [(X_train.shape[1], X_train.shape[1], X_train.shape[3])]
        ...
        for app_name in test_results:
            ...
            print("Training ", app_name, " in ", self.MODEL_NAME, " model\n", end="\r")
            model = self.create_model(test_results[app_name][1]['input_shape'])
