...
class SVM():
    def __init__(self, params):
        ...
        self.column = params.get('predicted_column', ("appliance_power", "apparent"))
    ...
    def partial_fit(self, trains_mains, train_appliances):
        ...
        if not self.randomsearch:
            ...
            for app_name, appliance_power in train_appliances:
                ...
                X_train = generate_main_timeseries(trains_mains, False, timewindow, timestep)
                ...
                if app_name in self.model:
                    ...
                    model = SVC(**self.params.get(app_name, self.default_params))
            ...
            results = self.random_search(trains_mains, train_appliances)
    ...
    def random_search(self, trains_mains, train_appliances):
        ...
        for app_name in test_results:
            ...
            X_train = generate_main_timeseries(trains_mains, False,test_results[app_name][2], test_results[app_name][3])
            X_train = X_train.reshape(X_train.shape[0], X_train.shape[1]*2)

            y_train = generate_appliance_timeseries(appliance_power, True, test_results[app_name][2], test_results[app_name][3], self.column)

            print("Training ", app_name, " in ", self.MODEL_NAME, " model\n", end="\r")
            model = SVC(**test_results[app_name][1])
