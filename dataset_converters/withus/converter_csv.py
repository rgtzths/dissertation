...
def load_period(base_url, device, beginning, end, username, password, is_meter):
    if is_meter:
        print(base_url + 'mono-meter/individual?device_id='+device+"&begin="+beginning+"&end="+end)
        response = StringIO(
        ...
        print(base_url + 'plug/individual?device_id='+device+"&begin="+beginning+"&end="+end)
        response =  StringIO (
...
def download_and_convert(base_url, username, password, appliances_mapping, timestep, interpolate, output_path):
    print(requests.get(base_url + 'households', auth=(username,password), timeout=None))
    ...
    for house in houses:
        ...
        for app in response:
            ...
            if app["device_type"] != "meter":
                #if "description" in app["metadata"]:
                #    print(app["metadata"]["description"])
        ...
        if load_data:
            for app in response:
                dfs = []
                print(app)
                if app["device"] not in ["688AB5015D8A", "00124B0003374C2B:55"]:
                    if app["device_type"] != "meter":
                        if "description" in app["metadata"] and app["metadata"]["description"] in appliances_mapping:
                            ...
                            end_date = begining_date + week_td*16
